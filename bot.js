...
bot.on("message", async ctx => {
  ...
  if (codes.includes(text)) {
    let isSubscribed = (ctx.session.isSubscribed == null || ctx.session.isSubscribed === false) ? checkSubscription(ctx) : ctx.session.isSubscribed;
    if (isSubscribed) {
      let repText = cfg.message_text
      await ctx.replyWithHTML(repText, {disable_web_page_preview: true})
      await handleFilesSending(ctx)
      await db.incUsage(text);
    } else {
      await onStartLogic(ctx)
    }
  } else {
    ctx.reply("Кодовое слово неправильное. Пожалуйста, введите корректное кодовое слово")
  }
})
bot.launch()
